...
def main():
    if date.today().weekday() in [3]:
        try:
            ...
            try:
                while datetime.now().time()<=time(14,00):
                    print('Time not 14.00pm waiting for 1min')
                    logging.info('Time not 14.00pm waiting for 1min')
                    sleep(60)
        ...
        print('Today is not market expiry')
        logging.info('Today is not market expiry')
